...
import { getFatura, enviarEmailFatura, downloadFaturaPdf } from '../services/faturas.js'

const props = defineProps({
  faturaId: { type: Number, required: true },
})
...
// email sub-state: 'idle' | 'input' | 'sending' | 'sent' | 'error'
...
onMounted(async () => {
  try {
    const { data } = await getFatura(props.faturaId)
    fatura.value = data.data
    emailInput.value = fatura.value.cliente?.email ?? ''
  } catch {
    loadErr.value = 'Erro ao carregar fatura.'
  } finally {
    loading.value = false
  }
})
...
async function handlePrint() {
  ...
  let url = null
  try {
    const { data } = await downloadFaturaPdf(props.faturaId)
...
function handleEmailClick() {
  if (fatura.value?.cliente?.email) {
    doSend(fatura.value.cliente.email)
  } else {
...
async function doSend(emailTo) {
  ...
  emailErr.value = ''
  try {
    await enviarEmailFatura(props.faturaId, { email: emailTo || null })
      ...
      <div class="modal">
        ...
        <div class="modal-header">
          <div class="header-left">
            <span class="modal-title">Fatura</span>
            <span v-if="fatura" class="modal-numero">{{ fatura.numero }}</span>
          </div>
        ...
        <div class="modal-body">
          <div v-if="loading" class="state-msg">A carregar...</div>
          <div v-else-if="loadErr" class="state-msg error">{{ loadErr }}</div>
          ...
          <div v-else class="preview-paper">
            ...
            <div class="inv-header">
              ...
              <div class="inv-meta">
                ...
                <span class="inv-estado" :class="fatura.estado === 'EMITIDA' ? 'estado--emitida' : 'estado--anulada'">
                  {{ fatura.estado }}
                </span>
              </div>
            </div>
            ...
            <p class="inv-footer">OS #{{ fatura.ordem_servico_id }}</p>

          </div>
        </div>

        <!-- Footer: actions -->
        <div v-if="fatura" class="modal-footer">
          <div class="footer-actions">
            ...
            <!-- Email: idle — has email -->
            <button
              v-if="emailState === 'idle' && fatura.cliente?.email"
              class="action-btn"
            ...
            <!-- Email: idle — no email -->
            <button
              v-else-if="emailState === 'idle' && !fatura.cliente?.email"
              class="action-btn"
              @click="handleEmailClick"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
            ...
            <div v-else-if="emailState === 'input'" class="email-input-row">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
              <input
                v-model="emailInput"
                type="email"
                class="email-field"
                placeholder="Email do cliente"
                @keydown.enter="doSend(emailInput)"
              />
              <button class="action-btn action-btn--compact" @click="doSend(emailInput)" :disabled="!emailInput.trim()">
                Enviar
              </button>
...
.estado--anulada { background: #fee2e2; color: #b91c1c; }
...
.ref      { font-family: 'Courier New', monospace; font-size: 0.78rem; color: #6b7280; }
...
.action-btn--ghost:hover { background: #f3f4f6; border-color: transparent; }
